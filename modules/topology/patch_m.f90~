...
module Patch
    ...
    subroutine addChild( object, child)
        ...
        ! Test wether the object has children or the number of children is equal to zero. Then the first children can be moved inside the object without a new allacation.
        ...
        if(testSz) then
            ...
            ! Move the new pointer to the object and deallocate the previous array
    ...
    subroutine addNeighbour( object, neighbour )
        ...
        testSz = associated(object%neighbours) .or. object%numberOfNeighbours .eq. 0
        if( testSz ) then
